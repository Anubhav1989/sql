...
select code,name,continent,population,
sum(population) over() as total_population,sum(population)
over(partition by continent) as continent_population from country;

select code,name,continent,population,region,
sum(population) over(partition by continent),sum(population)
over(partition by continent,region) from country;
...
select *,rank() over(order by city),
