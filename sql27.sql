...
select * from employees join departments on employees.dept_id=
departments.dept_id where dept_name='IT' or dept_name='Finance';
...
employees as e join departments as d on e.dept_id= d.dept_id 
where d.dept_name='HR');

select * from employees where salary =any(select salary from employees
as e join departments as d on e.dept_id=d.dept_id where 
dept_name='Sales');

select * from employees as e1 where salary>(select avg(salary) from 
employees as e2 where e2.dept_id=e1.dept_id);
...
select * from employees as e1 where salary=(select max(salary) from
employees as e2 where e2.dept_id=e1.dept_id);

select d.dept_id,avg(e.salary) from departments as d join employees as e 
on d.dept_id=e.dept_id where e.salary>(select avg(salary) from employees)
group by d.dept_id;
...
having total>=all(select sum(salary) as total from employees
group by dept_id);

select dept_id,min(salary) as min_s from employees group by dept_id
having min_s>(select min(salary) from employees join departments
on departments.dept_id=employees.dept_id where dept_name ='Sales');
