...
select * from students as s join enrollments as e 
on s.student_id=e.student_id;

select * from students as s left join enrollments as e
on s.student_id=e.student_id; 

select * from students as s right join enrollments as e
on s.student_id=e.student_id;

select s.student_name,count(e.course_name) from students as s 
...
select s.student_id,s.student_name,count(e.course_name)
from students as s join enrollments as e on s.student_id=e.student_id 
group by s.student_id having count(e.course_name)>2;

select major,count(student_id) from students group by major;

select s.major,sum(e.credits) from students as s join enrollments as e
on s.student_id=e.student_id group by s.major;

select upper(s.student_name),count(e.enrollment_id) from students as s 
join enrollments as e on s.student_id=e.student_id group by s.student_name;

select char_length(s.student_name),count(e.enrollment_id) 
from students as s join enrollments as e on
s.student_id=e.student_id group by char_length(s.student_name);
